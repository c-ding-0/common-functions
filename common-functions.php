...
function common_scripts(){
    ...
    wp_register_script('markitup-settings',$library_url."editor/markitup/sets/my-settings/set.js",array('jquery','jquery-ui-dialog','jquery-ui-tabs','dialog','citation-js'),$version,true);
 ...
 * default pages
 */
function insert_default_page($args){
    $args=array_merge(array('post_type'=>'page','post_status'=>'publish'),$args);
    //$post_id=post_exists($args['post_title'],'','',$args['post_type'],$args['post_status']);
    $post_id=get_page_by_path($args['post_name'])->ID;
    if(!$post_id){
        $post_id= wp_insert_post($args);
    }
    return $post_id;
}
function  get_default_page_permalink($post_title){    
    return get_permalink(get_page_by_path($post_title)->ID);
}
...
function insert_edit_page(){
    ...
    $post_id=insert_default_page($args);
    return $post_id;
}
...
function comment_menus(){
    ...
    echo "<div class='comment-menus menus hidden'>";
...
function get_comment_vote_link_text($comment_id){
    ...
    return "<span class='text'>$vote_html</span><span class='number bracket'>$votes_number_html</span>";
}
